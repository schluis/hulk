...
class TestRigidSpeed(unittest.TestCase):
    def test_speed(self) -> None:  # noqa: C901
        n_envs = 30000
        n_frame_fps = 10

        def run_anymal_c(solver: gs.constraint_solver) -> str:
            gs.init(
                backend=gs.gpu,
            ...
            motor_dofs = [
                robot.get_joint(name).dof_idx_local for name in joint_names
            ]
            ...
            result = (
                f"anymal \t| {solver} \t| {total_fps:,.2f} fps \t| {n_envs} env"
            )
            gs.destroy()

            return result

        def run_batched_franka(solver: gs.constraint_solver) -> str:
            gs.init(
                backend=gs.gpu,
                logging_level="warning",
                precision="32",
            ...
            total_fps = 1.0 / (1.0 / np.array(vec_fps[-n_frame_fps:])).mean()
            result = (
                f"frank \t| {solver} \t| {total_fps:,.2f} fps \t| {n_envs} envs"
            )

            gs.destroy()
            return result

        def run_random(solver: gs.constraint_solver) -> str:
            gs.init(
                backend=gs.gpu,
                logging_level="warning",
                precision="32",
            ...
            robot.control_dofs_position(
                torch.zeros((n_envs, 12), device="cuda"),
                dofs,
            )
            for _ in range(1000):
                robot.control_dofs_position(
                    torch.rand((n_envs, 12), device="cuda") * 0.1 - 0.05,
                    dofs,
                )
                scene.step()
                vec_fps.append(scene.FPS_tracker.total_fps)
