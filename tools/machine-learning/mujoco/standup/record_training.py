...
if __name__ == "__main__":
    ...
    checkpoints_names = list(
        filter(lambda x: x.startswith("rl_model_"), models_dir_entries)
    )
    checkpoints = list(
        map(lambda x: int(re.findall(r"\d+", x)[0]), checkpoints_names)
    )
    checkpoints.sort()
    ...
    for checkpoint in checkpoints:
        ...
        assert (
            return_code == 0
        ), f"Failed to record the {checkpoint} checkpoint model"
    ...
    episode_videos_names = (
        checkpoints_videos_names
        + final_model_video_name
        + best_model_video_name
    )
    episode_videos_path = [
    ...
    episode_videos_names = list(
        map(get_text_from_video_filename, episode_videos_names)
    )
    ...
    tmp_videos_path = [
        path_[:-4] + "_with_text" + ".mp4" for path_ in tmp_videos_path
    ]
    ...
    final_video_path = os.path.abspath(
        os.path.join(video_folder, "training.mp4")
    )
    os.system(
    ...
    if convert_to_gif:
        final_gif_path = os.path.abspath(
            os.path.join(video_folder, "training.gif")
        )
        os.system(
