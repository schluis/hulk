...
def collect_sensor_data_from_mcap(
    path: Path | str,
    topic: str,
    ...
    with path.open("rb") as file:
        ...
        for _, _, message in tqdm.tqdm(messages, desc=f"collecting {topic}"):
            ...
            flattened.update(
                {
                    f"inertial_measurement_unit.{quantity}.{dimension}": value
                    for quantity in ["linear_acceleration", "angular_velocity"]
                    for value, dimension in zip(
                        data["inertial_measurement_unit"][quantity],
                        ["x", "y", "z"],
                        strict=False,
                    )
                }
            ...
            flattened.update(
                {
                    f"force_sensitive_resistors.{side}": sum(
                        data["force_sensitive_resistors"][side].values()
                    )
            ...
            restructured = np.array(
                [flattened[name] for name in sensor_names],
                dtype=np.float64,
            )
...
def collect_positions_from_mcap(
    path: Path | str,
    topic: str,
...
def load_recorded_positions(
    recording: str,
    sensors: Sequence[str],
    *,
    max_num_messages: int | None = None,
) -> npt.NDArray[np.float64]:
    return collect_positions_from_mcap(
        recording,
...
def load_recorded_sensor_data(
    recording: str,
    sensors: Sequence[str],
    *,
    max_num_messages: int | None = None,
) -> npt.NDArray[np.float64]:
    return collect_sensor_data_from_mcap(
        recording,
        "Control.main_outputs.sensor_data",
        sensors,
        max_num_messages,
